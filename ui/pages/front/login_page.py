...
class FrontLoginPage(BasePage):
    ...
    def _ensure_agreement_checked(self) -> None:
        ...
        # 显式等待协议复选框渲染出现，避免 implicit_wait=0 时未渲染就返回空导致漏勾选、登录被拦（项目：ui/pages/base_page.py → BasePage.elements_present）
        for checkbox in self.elements_present(*self.AGREE_CHECKBOX):
            ...
            # 必须点击 Element Plus 复选框方块 .el-checkbox__inner 才能真正切换；直接点 label 会落在文字/协议链接上而不勾选（第三方：selenium → find_element, By.CSS_SELECTOR）
            try:
